		...
		try {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					case `/${userID_Path}`: {
						const vlessConfig = getVLESSConfig(userID_Path, request.headers.get('Host'));
...
function getVLESSConfig(userID, hostName) {
	...
	const clashLink = `https://subconverter.do.xn--b6gac.eu.org/sub?target=clash&url=https://${hostName}/sub/${userID}?format=clash&insert=false&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
	return `UUID: ${userID}<br>
---------------------------------------------------------------<br>
<a href="//${hostName}/sub/${userID}" target="_blank">VLESS节点订阅</a><br>
<a href="clash://install-config?url=${encodeURIComponent(clashLink)}" target="_blank">Clash节点订阅</a><br>
<a href="${clashLink}" target="_blank">Clash节点订阅2</a>
<pre style="width:80%; word-break:break-all; white-space:pre-wrap;">
################################################################
...
proxy-providers:
  provider1:
	type: http
	url: https://${hostName}/sub/${userID}?format=clash
	interval: 3600
	path: ./provider1.yaml
	health-check:
	  enable: true
...
function createVLESSSub(userIDs, hostName) {
	...
	let userIDArray = userIDs.includes(',') ? userIDs.split(',') : [userIDs];
